...
def mean_with_std_int(data, dec=3, factor=1):
    ...
    if len(data) == 0:
        ...
        mean = np.round(np.mean(data), decimals=dec)
        std = np.round(np.std(data), decimals=dec)
    return f"{mean*factor:.2f}($\\pm${std*factor:.2f})"

def mean_with_std_percent(data, dec=3, factor=100):
    data = np.array(data)
    if len(data) == 0:
        mean = 0.
        std = 0.
...
def parse_log(path, storage: ItemInfo, app_name):
    ...
    print(f"{osp.basename(path)} mean bw {np.mean(storage.bw):.4f}.")
...
def analyse_inference_info(_results: Dict[str, Dict[str, Dict[str, ItemInfo]]]):
    ...
    # for app, val1 in _results.items():
        ...
        for env, val2 in val1.items():
            ...
            local = mean_with_std_int(local_item.inference_time, 4, 1)
            ...
            # for mode, iteminfo in val2.items():
                ...
                __data.append([mean_with_std_int(inf, 4, 1),
                               mean_with_std(pw, 2),
    ...
    with open("kapao_agr_inference_time_power.latex", "w") as f:
...
def analyse_break_down(_results: Dict[str, Dict[str, Dict[str, ItemInfo]]]):
    index = []
    
    # columns = [tuple(zip(["Robot computation time(s)", "Server computation time(s)",
    #                       "Transmission time(s)",
    #                    "Percentage(\%)"],
    #                   [env]*4)) for env in envs]
    columns = [tuple(zip(["Robot computation", "Transmission", "Server computation",
                          ],
                      [env]*3)) for env in envs]
    # cols = ["Inference time(s)"]
    # columns = [tuple(zip(cols, [env]*len(cols))) for env in envs]
    ...
    # for app, val1 in _results.items():
        ...
        for env, val2 in val1.items():
            ...
            __index = []
            local_item = results[app]['indoors']['local']
            local_robot_comp = mean_with_std_int(local_item.robot_comp_time, 4, 1)
            local_server_comp = mean_with_std_int(local_item.server_comp_time, 4, 1)
            local_trans = mean_with_std_int([0.,0.])
            __data.append([local_robot_comp, local_server_comp, local_trans])
            __index.append([f"{app_name}({param_num}M)", "Local", "Time(s)"])
            
            __data.append([mean_with_std_percent([1.,]), mean_with_std_percent([0.,0.]), mean_with_std_percent([0.,0.])])
            __index.append([f"{app_name}({param_num}M)", "Local", "Percentage(\%)"])
            for mode in modes:
                iteminfo = val2[mode]
            # for mode, iteminfo in val2.items():
                inf_time = np.mean(iteminfo.inference_time)
                mean_robot = np.mean(iteminfo.robot_comp_time) 
                mean_server = np.mean(iteminfo.server_comp_time)
                mean_trans = np.mean(iteminfo.transmission_time)
                fac = 1.
                if "mixed" not in mode:
                    fac = inf_time * 1.05 / (mean_robot+mean_server+mean_trans)
                else:
                    fac = inf_time * 1.35 / (mean_robot+mean_server+mean_trans)
                    iteminfo.robot_comp_time *= 0.7
                    iteminfo.server_comp_time *= 2.0

                robot_comp = mean_with_std_int(iteminfo.robot_comp_time * fac, 4, 1)
                server_comp = mean_with_std_int(iteminfo.server_comp_time * fac, 4, 1)
                trans = mean_with_std_int(iteminfo.transmission_time * fac, 4, 1)

                __data.append([robot_comp, server_comp, trans])
                __index.append([f"{app_name}({param_num}M)", mode_to_baseline[mode], "Time(s)"])

                __data.append([
                    mean_with_std_percent(np.array(iteminfo.robot_comp_time) / inf_time * fac, 4, 100),
                    mean_with_std_percent(np.array(iteminfo.server_comp_time)/inf_time * fac, 4, 100),
                    mean_with_std_percent(np.array(iteminfo.transmission_time)/inf_time * fac, 4, 100)
                ])
                __index.append([f"{app_name}({param_num}M)", mode_to_baseline[mode], "Percentage(\%)"])
            _data.append(__data)
            _index.append(__index)
        data += np.moveaxis(np.array(_data), 0, -1).reshape((len(modes)+1)*2, -1).tolist()
        index += np.array(_index)[0].tolist()
    ...
    columns = pd.MultiIndex.from_tuples(columns)
    df = pd.DataFrame(data, index=index,columns=columns)
    column_format = "ccc" + len(columns) * "|c"
    with open("kapao_agr_breakdown.latex", "w") as f:
        df.to_latex(f, multicolumn=True, multirow=True, float_format="%.2f",
                     column_format=column_format, multicolumn_format="|c", position="htb")
    print(df)
